...
% cell = 1;
range = (align-10:align+30);
prange = ((align-60):(align+80));
counter1 =1;
counter2 = 1;
...
%     dt = calcium{cell}(i,locs+align-10);
 dt = mean(mean(calcium{cell}(i,range),1));
    Y = prctile(calcium{cell}(i,prange),1);
    if Motif_ind(3,i) == 0; % directed
%     valU(:,i) = (dt-Y)-(pks-Ym);
%     valU2(:,i) = NaN;
    vTD(1,counter1) = ((dt-Y)-(pks-Ym))-1;
    vTD(2,counter1) = cell;
...
% legend('Dir', 'Undir')
title( 'Trace Amp difference, compared to avg, for all cells in one bird (Dir = red)')
ylabel('probability')
