...
% zscore data
for i = 1:size(D2.unsorted,3); D2.unsorted_z(:,:,i) = zscore(D2.unsorted(:,:,i)')';end;
...
[    c(:,i), s(:,i), options(:,i)] = deconvolveCa(mean(D2.unsorted_z(:,:,i),1)-min(mean(D2.unsorted_z(:,:,i),1)));
  end
...
% check it
  figure();
    for i = 1: 40;
      hold on;
      plot(mean(D2.unsorted_z(:,:,i),1)); hold on; plot(c(:,i)); plot(s(:,i));
      pause();
      clf;
    end
...
cd('processed')
disp('saving data...')
save('out_data','out','-v7.3');
save('input_data','c','s','Gconsensus3','FS_ROI_Data','D2','f','t','fs','options','-v7.3');
