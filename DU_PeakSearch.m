function [pvalD,A1sort,A2sort,Bsort, B_unsort,B_rand, out] = DU_PeakSearch(data);
...
for i = 1:size(s,2); % for every cell
    ...
    for ii = 1:size(idx);
        ...
        try
            DffHeight(:,counter) = max(squeeze(data.all(:,idx(ii):idx(ii)+frames,i))'-min(squeeze(data.all(:,:,i))'));
...
%figure(); hold on; plot(mean(In_b),mean(In_a),'o'); line([-4 4],[-4 4]);

out.DffHeight = DffHeight;
out.DffHeight_time = DffHeight_time;
...
Bsort = B(tb,tb);
B_unsort = B;
B_rand = B(randi(max(tb),max(tb),1),randi(max(tb),max(tb),1));
