...
% PCompute statistic for trial-trial amplitude Differences for each cell
alphaVal = 0.05./(size(data.directed,3)*2);
for i = 1: size(data.directed,3) % for all cells
GD = max(data.directed(:,:,i),[],2)-min(data.directed(:,:,i),[],2);
GU = max(data.undirected(:,:,i),[],2)-min(data.undirected(:,:,i),[],2);
[pvalD(i),HH1(i)] = ranksum(GU, GD,'alpha', alphaVal,'tail','right');
[pvalU(i),HH2(i)] = ranksum(GU, GD,'alpha', alphaVal,'tail','left');
clear GD GU
end
SigUndirected = sum(HH1)./(size(data.directed,3))*100
SigDirected = sum(HH2)./(size(data.directed,3))*100
Msig(:,1) = HH1;
Msig(:,2) = HH2;
